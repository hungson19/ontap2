package ontap.decriptionMethod;

import ontap.desgine.PMethod;
import ontap.entity.Product;
...
public class ProductMethod implements PMethod {
    ...
    public Product inputData() {
        ...
        for (int i = 0; i < CategoriesMethod.count; i++) {
            System.out.println(CategoriesMethod.categories[i].toString());;
        }
        System.out.println("chọn ID categories");
        int chooseID = Integer.parseInt(scanner.nextLine());
        for (int i = 0; i < CategoriesMethod.count; i++) {
            if (CategoriesMethod.categories[i].getCatalogId() == chooseID) {
                product.setCatalogId(CategoriesMethod.categories[i].getCatalogName());
            }
        }
